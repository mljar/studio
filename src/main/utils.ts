    ...
    installerProc.on('exit', (exitCode: number) => {
      if (exitCode === 0) {
        ...
        const pocInstallCommand = isWin
          ? `${installPath}\\python.exe -m pip install --force-reinstall ${installPath}\\pieceofcode-0.3.2-py3-none-any.whl`
          : `${installPath}/bin/python3 -m pip install --force-reinstall ${installPath}/pieceofcode-0.3.2-py3-none-any.whl`;
